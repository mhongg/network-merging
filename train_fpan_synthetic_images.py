...
def load_dataset(args, paths, seed, channels=1):
    ...
    datasets = []
    for path in paths:
        path = args.data_dir + path + "/" + str(seed) 
        if channels == 1: 
            datasets.append(ImageFolder(root=path, transform=transform))
        elif channels == 3:
            datasets.append(ImageFolder(root=path, transform=transform_rgb))
...
def train_fpan(args):
    ...
    for i in range(len(args.seeds)):
        ...
        if args.fpan_data == "disjoint_mnist":
            train_loaders = [
                load_dataset(args, ["Synthetic first5_mnist", "Synthetic last5_mnist"], args.seeds[i]),
                load_dataset(args, ["Synthetic first5_mnist", "Synthetic last5_mnist"], args.seeds[i]),
            ]
            ...
            train_loaders = [
                load_dataset(args, ["Synthetic mnist", "Synthetic cifar10"], args.seeds[i]),
                load_dataset(args, ["Synthetic mnist", "Synthetic cifar10"], args.seeds[i], channels=3),
            ]
            ...
            train_loaders = [
                load_dataset(args, ["Synthetic fmnist", "Synthetic kmnist"], args.seeds[i]),
                load_dataset(args, ["Synthetic fmnist", "Synthetic kmnist"], args.seeds[i]),
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_dir", type=str, default="./results/experiments/fpan_trained_with_synthetic_images/")
    ...
    parser.add_argument("--output_dir", type=str, default="./cache/models/experiments/fpan_trained_with_synthetic_images/")
