# Acquired and modified from https://github.com/greentfrapp/lucent/blob/dev/lucent/optvis/render.py
#
...
def generate_images(args):
    ...
    for i in range(len(args.seeds)):
        ...
        # Load model
        model = arch(
            input_channel=args.input_channel, output_size=args.output_size
        ).to(device)
        model.load_state_dict(
            torch.load(
                args.model_dir +
                f"{args.dataset}_{args.arch}_{args.seeds[i]}",
        ...
        for label in range(args.output_size):
            ...
            if label == args.output_size - 1:
                nb_images = int(args.size/args.output_size + args.size % args.output_size)
            else:
                nb_images = int(args.size/args.output_size)

            # Create the directory for saving if it does not exist
            create_op_dir(args.data_dir + "Synthetic " + args.dataset + "/" + str(args.seeds[i]) + "/" + str(label) + "/")

            for idx in range(nb_images):
                param_f = lambda: param.image(32, decorrelate=True, fft=True, channels=args.input_channel)
                transforms = [
                    transform.pad(4),
                    transform.jitter(2),
                ...
                render_vis(
                    ...
                    + "/cache/data/" + "Synthetic " + args.dataset + "/" + str(args.seeds[i]) + "/" 
